...
// This is the final "test cases" who will execute an infinite logging, until user close the test runner UI
HANDLE g_logSampleThread = NULL;
bool g_shouldExitLogSampleThread = false;
TPUT_DEFINE_BLOCK(L"Log Usage Example", L"")
{
	// Create a new thread to demonstrate logging usage
	struct inner
	{
		static DWORD WINAPI ThreadProc(LPVOID /*lpParameter*/)
		{
			...
			ctrl->AddOutputDevice(L"file", LODT_FILE, L"enable:true path:'${EXE_DIR}\\sample.log'");
			int count = 0;
			while (!g_shouldExitLogSampleThread)
			{
				Log(LL_EVENT, TAG_DEFAULT, L"test log %d", count++);
				::Sleep(1000);
			}
			return 0;
		}
	};
	g_logSampleThread = ::CreateThread(NULL, 0, inner::ThreadProc, NULL, 0, NULL);
	TPUT_EXPECT(g_logSampleThread != NULL, L"Sample log started. check `sample.log` or open `tplogview` to connect to the log.");
}
...
int WINAPI wWinMain(HINSTANCE /*hInstance*/, HINSTANCE /*hPrevInstance*/, LPWSTR /*lpstrCmdLine*/, int /*nCmdShow*/)
{
	...
	g_shouldExitLogSampleThread = true;
	::WaitForSingleObject(g_logSampleThread, INFINITE);
