...
namespace tp
{
    namespace oswin
    {
        ...
        static inline std::wstring get_module_path(HMODULE m)
        {
            wchar_t buffer[4096];
        ...
        static inline bool is_path_seperator(wchar_t ch)
        {
            return ch == L'\\' || ch == L'/';
        }

        static inline std::wstring parent_path(const std::wstring& path)
        {
        ...
        static inline UINT64 file_size(const std::wstring& file)
        {
         ...
         */
        static inline bool is_elevated()
        {
            HANDLE hToken;
        ...
        struct reg
        {
            static void reg_read_value(HKEY root, const std::wstring& path, const std::wstring& name, DWORD& type, LPBYTE buffer, LPDWORD buflen)
            {
                HKEY hkey;
                LSTATUS ret;
            ...
            static std::wstring reg_read_string(HKEY root, const std::wstring& path, const std::wstring& name, const std::wstring& defval)
            {
                WCHAR buffer[1024] = {};
            ...
            static DWORD reg_read_dword(HKEY root, const std::wstring& path, const std::wstring& name, const DWORD defval)
            {
                DWORD buffer = 0;
                DWORD buffer_len = sizeof(buffer);
                DWORD type = 0;
            ...
            static void reg_copykey(HKEY src, HKEY dest)
            {
            ...
            static std::vector<std::wstring> reg_get_subkeys(HKEY root, const std::wstring& path)
            {
            ...
            static void reg_write_dword(HKEY root, LPCWSTR path, LPCWSTR name, DWORD value)
            {
                HKEY hkey;
            ...
            static void reg_deletekey(HKEY root, const std::wstring& pathname)
            {
            ...
            static void reg_deletekey(HKEY root, const std::wstring& path, const std::wstring& name)
            {
                HKEY hkey;
