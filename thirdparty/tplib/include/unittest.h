...
namespace tp
{
    ...
    public:
        ...
        void add_test_block(const wchar_t* name, const wchar_t* tags, void (*func)())
        {
            ...
            // blocks starting with "#" are excluded from running
            if (name[0] == L'#') return;
...
//        void run_test(const wchar_t* tags)          { return run_test(0, NULL, tags); }
        ...
        static unittest& instance()
        {
            ...
            /// warning C4640: constructing local static variable is not thread-safe
            /// in unittest senaro, this function is triggered by global objects used to register test cases, 
            /// on this registering phase, there's only one thread exists,
